...
static FILE *pdfi;       /* File descriptor for pdef file */
 ...
 gaint ccsddes (char **pstrCTL, struct gafile *pfi, gaint mflag) 
 {
	 if(pstrCTL== NULL)
	 {
		 gaprnt (0,"pointer to description file is NULL.\n");
		 return(1);
	 }
	 ...
	 /********** Parse the data descriptor file  ************************************************/
	  int nRow = 0;
	 while(strcmp(pstrCTL[nRow],"") )
	 {
		  /* Remove any leading blanks from rec */
		 strcpy(rec, pstrCTL[nRow]);
         reclen = strlen(rec);
         jj = 0;
	...
	*(pfi->scattr+len) = '\0';
	...
	for (j=0;j<2;j++) {
	  ...
	  pfi->bufrinfo->offset.hrxy[j] = pfi->bufrinfo->offset.mnxy[j] = -999;;
		...
		printf("Open Error:  Data file type invalid\n");
        //gaprnt (0,"Open Error:  Data file type invalid\n");
        goto err9;
      }
	...
	if (pfi->bufrinfo->stidxy[0]==-999 || pfi->bufrinfo->stidxy[1]==-999) goto err6a;
      ...
      if (ch==NULL) {
        ...
        goto err9;
		  ...
		  if (*ch=='^' || *ch=='$') 
		  {
			//fnmexp (pfi->name,ch+1,name);
			  strcpy(pfi->name,ch+1);
		  } 
		  else 
		  {
      ...
      if (*ch=='^' || *ch=='$') 
	  {
        fnmexp (pout, ch, pfi->name);
	     //strcpy(pfi->mnam,ch+1);
		  //pfi->mnam = ch+1;
      } 
	  else 
	  {
	...
	*(pfi->undefattr+len) = '\0';
	...
	ch = mrec + (ch-rec);
        if (*ch=='^' || *ch=='$') {
		  ...
		  getwrd (pdefnm,ch+1,255);
        } else {

          getwrd (pdefnm,ch,256);
        }
	/* open the pdef file */
        fopen_s(&pdfi, pdefnm,"rb");
        if (pdfi==NULL) {
	...
	pfi->dnum[2] = 1;
      ...
      if (cmpwrd("linear",ch)) {
		...
		rc = ccsdeflev(ch,rec,pfi,2,pstrCTL,&nRow);
        if (rc==-1) goto err8; 
        if (rc) goto err9;
      } else goto err2;
      flgs[2] = 0;
  ...
  if (pfi->ens1==NULL) {
    ...
    while (j<pfi->dnum[4]) {
      ...
      ens->gt = (gaint)t2gr(pfi->abvals[3],&(ens->tinit));
...
gaint gaddes (char *name, struct gafile *pfi, gaint mflag) 
{
 ...
 /*mf --- define here vice grads.c for cdunif.c mf*/
  ...
  if (descr == NULL) {
    ...
    for(i=0;i<=(gaint)strlen(name);i++) *(sname+i)=*(name+i);
    /*strcat(sname,".ctl");*/
    fopen_s (&descr, sname, "r");
  }

  /* If still can't open descriptor file, give up */
  if (descr == NULL) {
	  gaprnt (0,"Open Error:  Can't open description file\n");
	  if (sname) gree((char*)sname,"f172");
	  return(1);
  }
...
gaint ccsdeflev (char *ch, char *rec, struct gafile *pfi, gaint dim, char **pstrCTL, int *nRow)
{
	...
	*vvs = (gadouble)pfi->dnum[dim];
	...
	for (i=0; i<pfi->dnum[dim]; i++) {
		if ( (ch = nxtwrd(ch))==NULL) {
			if (!strcmp(pstrCTL[*nRow+1],"")) goto err2;
			strcpy(rec, pstrCTL[*nRow+1]);
			*nRow = *nRow + 1;
			ch = rec;
			while (*ch==' ' || *ch=='\t') ch++;
			if (*ch=='\0' || *ch=='\n') goto err3;
...
struct gaattr *parseattr (char *ch) {
  ...
  if (nctype <= 2) {
    ...
    if ((dvalues = (double*)(void *)galloc(sz,"valued")) == NULL) {
       gaprnt (0,"Error: memory allocation failed for attribute metadata\n");
       goto err;
    }
    value = (void*)dvalues;
...
size_t sz;
  ...
  if (fvals!=NULL) gree((char*)fvals,"f80g");
  return(0);
...
err:
  if (pfi->ppi[0]!=NULL) gree((char*)pfi->ppi[0],"f80a");
  if (pfi->ppf[0]!=NULL) gree((char*)pfi->ppf[0],"f80c");
  if (pfi->ppf[1]!=NULL) gree((char*)pfi->ppf[1],"f80d");
  if (pfi->ppwrot && pfi->ppw!=NULL) gree((char*)pfi->ppw,"f80e");
  if (fvals!=NULL) gree((char*)fvals,"f80f");
  return(1);
...
*/
  ...
  gadouble pi_dd, pi2, pi4, d2r, r2d, radius, omega4;
...
void ll2rotll( gadouble *vals, gadouble grdlat, gadouble grdlon,
  ...
  const gadouble pi_dd = 4.0*atan( 1.0 );
...
void ll2pse (gaint im, gaint jm, gadouble *vals, gadouble lon, gadouble lat,
  ...
  const gadouble pi_dd = 3.141592654;
...
/* these are listed in the order in which they appear in the pfi declaration in grads.h */
  ...
  if (pfi->mnam)   gree((char*)pfi->mnam,"f57");
  if (pfi->rbuf)   gree((char*)pfi->rbuf,"f58");
  if (pfi->sbuf)   gree((char*)pfi->sbuf,"f58");
  if (pfi->pbuf)   gree((char*)pfi->pbuf,"f59");
  if (pfi->bbuf)   gree((char*)pfi->bbuf,"f60");
  if (pfi->ubuf)   gree((char*)pfi->ubuf,"f61");
  if (pfi->tstrt)  gree((char*)pfi->tstrt,"f62");
  if (pfi->tcnt)   gree((char*)pfi->tcnt,"f63");
  if (pfi->pvar1)  gree((char*)pfi->pvar1,"f64");
  if (pfi->ens1)   gree((char*)pfi->ens1,"f65");
  for (i=0; i<9; i++) if (pfi->ppi[i]) gree((char*)pfi->ppi[i],"f66");
  for (i=0; i<9; i++) if (pfi->ppf[i]) gree((char*)pfi->ppf[i],"f67");
  if (pfi->ppw) gree((char*)pfi->ppw,"f68");
  if (!flag) for (i=0; i<5; i++) {
    if (pfi->grvals[i]) gree((char*)pfi->grvals[i],"f69");
  }
  if (pfi->pindx) {
    pindx = pfi->pindx;
    if (pindx->hipnt)  gree((char*)pindx->hipnt,"f70");
    if (pindx->hfpnt)  gree((char*)pindx->hfpnt,"f71");
    if (pindx->intpnt) gree((char*)pindx->intpnt,"f72");
    if (pindx->fltpnt) gree((char*)pindx->fltpnt,"f73");
    gree((char*)pindx,"f74");
  }
  if (pfi->pindxb) {
    pindxb = pfi->pindxb;
    if (pindxb->bigpnt)  gree((char*)pindxb->bigpnt,"b98");
    gree((char*)pindxb,"b99");
  }
#if GRIB2
  if (pfi->g2indx) {
    g2indx = pfi->g2indx;
    if (g2indx->g2intpnt) gree((char*)g2indx->g2intpnt,"f75");
    if (g2indx->g2bigpnt) gree((char*)g2indx->g2bigpnt,"b75");
    gree((char*)g2indx,"f76");
  }
#endif
  if (pfi->fnums)  gree((char*)pfi->fnums,"f77");
  pchsub = pfi->pchsub1;
  while (pchsub) {
    if (pchsub->ch) gree((char*)pchsub->ch,"f78");
    pch2 = pchsub->forw;
    gree((char*)pchsub,"f79");
    pchsub = pch2;
  }
  if (pfi->scattr) gree((char*)pfi->scattr,"f80");
  if (pfi->ofattr) gree((char*)pfi->ofattr,"f81");
  if (pfi->undefattr) gree((char*)pfi->undefattr,"f82");
  if (pfi->bufrinfo) gree((char*)pfi->bufrinfo,"f83");
#ifndef STNDALN
  ...
  while (pfi->attr != NULL) {
    ...
    if (attrib->value) gree((char*)attrib->value,"f85");
    gree((char*)attrib,"f86");
  }
  gree((char*)pfi,"f87");
}
