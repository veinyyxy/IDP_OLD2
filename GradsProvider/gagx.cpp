...
extern QVector<double>  vector_ContorValue;
...
//static gaint rcols[13] = {9,14,4,11,5,13,3,10,7,12,8,2,6}; modifed by Liu Xinsheng 2011-06-20
static gaint rcols[15] = {9,14,4,11,5,13,3,10,7,12,8,2,6,254,255};  // 254对应色斑图值小于cmin的时的标色，255对应大于cmax时的标色
...
gaint gacntr (struct gacmn *pcm, gaint filflg, gaint shpflg) {
	...
	if (pgr->umin==0) {
		...
		{
			pcm->str_errorinfo =  new char [64];        //  added by Wang JiaLiang 2011-06-15
		}
		//strcpy(pcm->str_errorinfo,"Cannot contour grid - all undefined values.");
		strcpy(pcm->str_errorinfo,"无法绘制，全部值都是未定义值.");
		pcm->error = true;
	...
	if (!pcm->cflag) {
		...
		if (cint==0.0) {  /* field is a constant */
			if (pcm->dwrnflg && shpflg==0) {
				...
				strcpy(pcm->str_errorinfo,"无法绘制，全部值都是常量.");
				pcm->error = true;
	...
	if (filflg) {
		...
		if (pcm->cflag) {          /* user has specified contour levels */
			...
			for (rl=cmin;rl<=cmax+(cint/2.0);rl+=cint) {	/*Each contour WangJL*/
		...
		if (shpflg==0) {
			if (pcm->clab == 3) {
				...
				gxclab (pcm->clsiz,pcm->clab,pcm->clcol);	/*Picture frame and label WangJL*/
			}
			gxcrel ();
		}
	}
...
void gaselc (struct gacmn *pcm, gadouble rmin, gadouble rmax) {
	...
	if (!pcm->cflag) {
		...
		for (rr=cmin-cint;rr<=cmax+(cint/2.0);rr+=cint) {
			...
			pcm->shdlvs[lcnt] = rr;
			pcm->shdcls[lcnt] = 1;
			if (rr<pcm->cmin) pcm->shdcls[lcnt] = 254; //-1;   modified by Liu Xinsheng 2011-06-20
			if (rr+cint>pcm->cmax) pcm->shdcls[lcnt] = 255; //-1;  modified by Liu Xinsheng 2011-06-20
...
cleanup:
	fclose(outfile);
	gree((char*)varlevels,"f293");
	gree((char*)currpt,"f294");
	return;
}
		...
		if (pcm->cstyle!=0 && flag<2) {
			...
			for (i=1;i<=pgr->isiz;i++) {
				if (*gru == 0) {
					...
					vector_ContorValue.push_back(99999999.0f);    // added by Liu Xinsheng 2011-06-14
				...
				if (*gru != 0) {
					...
					if (flag==2) {   /* handle vectors/barbs */
						...
						if (*gr2u != 0 && *gr3u != 0) {
							...
							if (pcm->gout1a==2) {
								gxdrawwindbarb(x,y,dir,hypot(*gr2,*gr3));
								//gabarb (x, y, pcm->digsiz*1.5, pcm->digsiz*0.5,
								//pcm->digsiz*0.25, dir, hypot(*gr2,*gr3), hemflg);//zhaogaopan 5-18
							} else {
								if (vscal>0.0) {
			...
			if (rpt->umask != 0) {
				...
				if (rlon>pcm->dmin[0] && rlon<pcm->dmax[0] &&
					rpt->lat>pcm->dmin[1] && rpt->lat<pcm->dmax[1]) {
						...
						gxdrawtextvalue(lab,dx,dy);
						//gxdrawtextvalue(atoi(lab),x,y);
		...
		for (i=1; i<=pgru->isiz; i++) {
			if (*umask!=0 && *vmask!=0) {
				...
				if (lcol>-1) {
					...
					if (brbflg) {
						gxdrawwindbarb(x,y,dir+adj,hypot(*u,*v));
						//gabarb (x, y, pcm->digsiz*1.5, pcm->digsiz*0.5,
	...
	if (pcm->tser == 1) {
		...
		while (rpt!=NULL) {
			if (rpt->umask != 0 && rpt2->umask != 0) {
				...
				gxdrawwindbarb(x,y,dir,hypot(rpt->val,rpt2->val));
