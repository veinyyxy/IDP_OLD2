...
gaint gaggrd (struct gagrid *pgrid) {
  ...
  cflag = 0;
  if (bcflag) {
    gree((char*)bcache,"f106");
    gree((char*)bpcach,"f107");
  }
  bcache = NULL;
  bpcach = NULL;
  bcflag = 0;
...
gaint oldncopts ;         /* to save and restore setting for automatic error handling */
 ...
 cleanup:
  if (grtmp!=NULL)  gree((char*)grtmp,"f121");
  if (grutmp!=NULL) gree((char*)grutmp,"f122");
  if (grtmp2!=NULL) gree((char*)grtmp2,"f122a");
  ncopts = oldncopts ;
  return(rtrn);
...
gaint ccsread (off_t fpos, gaint len, gadouble *gr, char * gru)
{
	if( pfi->pArrayValuebuf == NULL) 
		{
			gaprnt (0,"The pointer to CCSDATA is NULL. \n");
			return (1);
	   }

	int i;
	gadouble *fData = pfi->pArrayValuebuf +fpos;
	
	//memcpy(gr,dccsData+fpos,len);       // Êý×é¹àÖµ
	for (i=0; i<len; i++) 
	{
		gr[i] = fData[i];
	}
...
gaint g2off,ng2elems=2;
 ...
 if (pfi->idxflg==1) {
   ...
   if (bpos>(off_t)-900 && bpos!=bpsav) {
     ...
     if (siz>bssav) {
       if (bcflag) {
	 gree((char*)bcache,"f114");
	 gree((char*)bpcach,"f115");
       }
       sz = siz;
