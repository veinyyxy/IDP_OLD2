...
void gagfre (struct gagrid *pgr) {
  ...
  if (pgr==NULL) return;
  if (pgr->alocf) {
    if (pgr->ivals != NULL) gree((char*)pgr->ivals,"f88");
    if (pgr->jvals != NULL) gree((char*)pgr->jvals,"f89");
  }
  if (pgr->idim>-1) {
    gree((char*)pgr->grid,"f90");
    gree((char*)pgr->umask,"f91");
  }
  gree((char*)pgr,"f92");
}
...
gaint i;
  if (stn==NULL) return;
  if (stn->tvals) gree((char*)(stn->tvals),"f237");
  if (stn->rpt) {
    for (i=0; i<BLKNUM; i++) {
      if (stn->blks[i] != NULL) gree((char*)(stn->blks[i]),"f238");
    }
  }
  gree((char*)stn,"f239");
}
