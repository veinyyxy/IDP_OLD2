...
gaint gxclvert (FILE *);
QVector<double>  vector_ContorValue;
...
static char pouts[256];           /* Build error msgs here */
...
static gadouble ldmin=1;    /* Minimum distance between labels */
...
void gxclev (gadouble *r, gaint is, gaint js, gaint ib, gaint ie, gaint jb,
	gaint je, gadouble v, char *u, struct gxcntr *pcntr) {
		...
		boundary.  This avoids having to test for the grid boundries
...
void gxcflw (gaint i, gaint j, gaint iside, gaint dr) {
	...
	*/
	...
	gaint isave,jsave;                     /* Save initial grid position    */
	gadouble *xy,*xyllim, *xyulim;         /* Buffer position and limits */ 


	isave=i; jsave=j;
	p1=rr+iss*(j-1)+i-1;                   /* Set pointers to corner values */
...
gaint gxcspl (gaint frombuf, struct gxcntr *pcntr) {
	...
	vector_ContorValue.push_back(pcntr->val);
	...
	if (pcntr->spline==0) {
		...
		else gxplot (*xystrt, *(xystrt+1), 3);	/*first point move*/
		for (xy=xystrt+2; xy<xyend-1; xy+=2) {
			gxplot (*xy, *(xy+1), 2);	/*next point draw*/
			if (!frombuf) {
				dacum += hypot(*xy-*(xy-2),*(xy+1)-*(xy-1));	/*The distance between two points*/
				if ((*(xy+1)-*(xy-1))*(*(xy+3)-*(xy+1))<0.01 &&		/*Inflection point*/
	...
	together.  If the contour is open, we extend the line straight
	...
	} while (d2<0.01);                     /* Loop till distance is big   */
	...
	} while (d1<0.01);                     /* Keep looping till d1 is big */
...
void gxpclab (gadouble xpos, gadouble ypos, gadouble rot, gaint ccol, struct gxcntr *pcntr) {
	...
	//set text value on the line zhaogp  
	//gadouble nx,ny;
	//gxvcon(x,y,&nx,&ny);
	gxdrawtextvalue(clabel,xpos,ypos);
	//gxdrawtextvalue(atoi(clabel),x,y);
	//--------------------------------
...
void gxpclin (void) {
	...
	while (pclbuf) {
		p2 = pclbuf->fpclbuf;
		if (pclbuf->lxy) gree ((char*)pclbuf->lxy,"c5");
		gree ((char*)pclbuf,"c6");
		pclbuf = p2;
	}
	clbufanch = NULL;
...
void gxclab (gadouble csize, gaint flag, gaint colflg) {
	...
	for (i=0; i<gxlabn; i++ ) {
		...
		if (gxlabs[i]==0.0) {
			...
			//gadouble nx,ny;
			//gxvcon(x,y,&nx,&ny);
			gxdrawtextvalue(gxlabv[i],gxlabx[i],gxlaby[i]);
			//gxdrawtextvalue(atoi(gxlabv[i]),x,y);
			//--------------------------------
...
void gxcrel (void) {
	if (lwk!=NULL) {
		gree ((char*)lwk,"c7"); lwk = NULL; 
	}
	if (fwk!=NULL) {
		gree ((char*)fwk,"c8"); fwk = NULL;
	}
	lwksiz = 0; fwksiz = 0;
}
...
gaint gxclvert (FILE *kmlfp)  {
	...
	while (pclbuf) { 
		if (pclbuf->lxy) {
			...
			snprintf(pouts,511,"  <Placemark>\n");
			if ((fwrite(pouts,sizeof(char),strlen(pouts),kmlfp))!=strlen(pouts)) {err=1; goto cleanup;}
			snprintf(pouts,511,"    <styleUrl>#%d</styleUrl>\n",pclbuf->color);
			if ((fwrite(pouts,sizeof(char),strlen(pouts),kmlfp))!=strlen(pouts)) {err=2; goto cleanup;}
			snprintf(pouts,511,"    <name>%g</name>\n",pclbuf->val);
			if ((fwrite(pouts,sizeof(char),strlen(pouts),kmlfp))!=strlen(pouts)) {err=3; goto cleanup;}
			snprintf(pouts,511,"    <LineString>\n");
			if ((fwrite(pouts,sizeof(char),strlen(pouts),kmlfp))!=strlen(pouts)) {err=4; goto cleanup;}
			snprintf(pouts,511,"      <altitudeMode>clampToGround</altitudeMode>\n");
			if ((fwrite(pouts,sizeof(char),strlen(pouts),kmlfp))!=strlen(pouts)) {err=5; goto cleanup;}
			snprintf(pouts,511,"      <tessellate>1</tessellate>\n");
			if ((fwrite(pouts,sizeof(char),strlen(pouts),kmlfp))!=strlen(pouts)) {err=6; goto cleanup;}
			snprintf(pouts,511,"      <coordinates>\n        ");
			if ((fwrite(pouts,sizeof(char),strlen(pouts),kmlfp))!=strlen(pouts)) {err=7; goto cleanup;}
			...
			for (i=0; i<pclbuf->len; i++) {
				...
				snprintf(pouts,511,"%g,%g,0 ",lon,lat); 
				if ((fwrite(pouts,sizeof(char),strlen(pouts),kmlfp))!=strlen(pouts)) {err=8; goto cleanup;}
				if (j==6 || i==(pclbuf->len-1)) { 
					if (j==6) snprintf(pouts,511,"\n        "); 
					else snprintf(pouts,511,"\n"); 
					if ((fwrite(pouts,sizeof(char),strlen(pouts),kmlfp))!=strlen(pouts)) {err=9; goto cleanup;}
					j=0;
				}
				j++;
			}
			...
			snprintf(pouts,511,"      </coordinates>\n    </LineString>\n  </Placemark>\n");
			if ((fwrite(pouts,sizeof(char),strlen(pouts),kmlfp))!=strlen(pouts)) {err=10; goto cleanup;}
			c++;
		}
		pclbuf = pclbuf->fpclbuf;
	}
