...
void gxshad (gadouble *r, gaint is, gaint js, gadouble *vs, gaint *clrs, gaint lvs, char *u) {
	...
	for (jmn=1; jmn<jmax; jmn+=3) {
		...
		for (k=0; k<lvs; k++) {
			...
			and where the grid boundaries are.  Flag values are:
			0 - nothing yet
			1 - contour has been drawn through this side
			7 - contour drawn through missing data box side
			8 - boundary between missing data value box and non-missing data value box
			...
			boundary (and missing-data-value boundaries) if necessary.      */
...
side4:                                 /* Enter side 4           */
	...
	/* At an undefined boundary and last moved towards the left.  */
