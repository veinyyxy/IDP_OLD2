...
int DBZDialog::gRowCount = 1;
DBZDialog::DBZDialog(int mix , int max ,QWidget *parent): QDialog(parent)
{
	ui.setupUi(this);
	...
	ui.pushBtnCal_0->setUserData(0, new IsoUserData);
}
...
/** 
  ...
  * @see references            
*/ 
void DBZDialog::Slots_PushButtonClicked()
{
	...
	else if (btn == ui.pushBtnAdd_0)//加号
	{
		...
		pushBtnCal->setUserData(0, new IsoUserData);
...
void IsoUserData::ComputerIsoSurface( int iValue )
{
	...
	ViewerQT* view = dynamic_cast<ViewerQT*>(SysMainWindow::GetMainWindow()->FromWidgetToQGLW());//SysMainWindow::GetMainWindow()->FromWidgetToQGLW();//->GetViewManger();
	if (view == NULL)
	{
		return;
	}
	OsgEarthBridge* pOEB = view->GetOsgEarthBridge();
	pOEB->ComputerIso(iValue);
	//pOEB->Computer3DClouds();
#if 0
	goto_gis::Map* map1 = view->Map2D();

	goto_gis::Layer* pCircleLayer = map1->GetLayer("Layer_Circle");
	...
	if (pCircleLayer != 0)
	{
		...
		goto_gis::Layer* pRadarLayer = map1->GetLayer("layer0");
...
void IsoUserData::DeleteIsoSuerface()
{
	...
	view->GetOsgEarthBridge()->DelIsoSuerface(m_iValue);
}
