...
ReaderWriter::ReadResult ReaderWriterGDAL::readObject(const std::string& file, const osgDB::ReaderWriter::Options* options) const
{
...
ReaderWriter::ReadResult ReaderWriterGDAL::readImage(const std::string& fileName, const osgDB::ReaderWriter::Options* options) const
{
	if (fileName.empty()) return ReadResult::FILE_NOT_FOUND;
...
ReaderWriter::ReadResult ReaderWriterGDAL::readHeightField(const std::string& fileName, const osgDB::ReaderWriter::Options* options) const
{
	if (fileName.empty()) return ReadResult::FILE_NOT_FOUND;
...
ReaderWriter::ReadResult ReaderWriterGDAL::local_readImage(const std::string& file, const osgDB::ReaderWriter::Options* options)
{
	...
	//set georef
	double oriCordL = geoTransform[0];  
	double oriCordB = geoTransform[3];     
	double RightX = geoTransform[0] + m_width*geoTransform[1] + m_height*geoTransform[2];
	double TopY = geoTransform[3] + m_width*geoTransform[4] + m_height*geoTransform[5];
	
	georef.flonmin = oriCordL<RightX?oriCordL:RightX;
	georef.flonmax = oriCordL>RightX?oriCordL:RightX;
	georef.flatmin = oriCordB<TopY?oriCordB:TopY;
	georef.flatmax = oriCordB>TopY?oriCordB:TopY;
	georef.m_width = m_width;
	georef.m_height = m_height;
...
ReaderWriter::ReadResult ReaderWriterGDAL::local_readHeightField(const std::string& fileName, const osgDB::ReaderWriter::Options* options)
{
...
void ReaderWriterGDAL::initGDAL() const
{
