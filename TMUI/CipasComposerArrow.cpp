...
CCipasComposerArrow::CCipasComposerArrow(CCipasComposition *composition)
...
CCipasComposerArrow::CCipasComposerArrow( const QPointF& startPoint, const QPointF& stopPoint, CCipasComposition* c )
	: CComposerItem( c )
...
CCipasComposerArrow::~CCipasComposerArrow(void)
{
}


void CCipasComposerArrow::initGraphicsSettings()
{
	setArrowHeadWidth( 4 );
...
void CCipasComposerArrow::paint( QPainter* painter, const QStyleOptionGraphicsItem* itemStyle, QWidget* pWidget )
{
	if ( !painter )
	{
		return;
...
void CCipasComposerArrow::setSceneRect( const QRectF& rectangle )
{
...
void CCipasComposerArrow::drawHardcodedMarker( QPainter* p, MarkerType type )
{
...
void CCipasComposerArrow::drawSVGMarker( QPainter* p, MarkerType type, const QString& markerPath )
{
	...
	//make nicer preview
	if ( mComposition && mComposition->plotStyle() == CCipasComposition::Preview )
	{
...
void CCipasComposerArrow::setStartMarker( const QString& svgPath )
{
	QSvgRenderer r;
	if ( !r.load( svgPath ) )
	{
...
void CCipasComposerArrow::setEndMarker( const QString& svgPath )
{
	QSvgRenderer r;
	if ( !r.load( svgPath ) )
	{
...
void CCipasComposerArrow::setOutlineWidth( double width )
{
	mPen.setWidthF( width );
	adaptItemSceneRect();
}

void CCipasComposerArrow::setArrowHeadWidth( double width )
{
...
void CCipasComposerArrow::adaptItemSceneRect()
{
	...
	CComposerItem::setSceneRect( rect );
}

bool CCipasComposerArrow::writeXML( QDomElement& elem, QDomDocument & doc ) const
{
...
bool CCipasComposerArrow::readXML( const QDomElement& itemElem, const QDomDocument& doc )
{
	...
	mMarkerMode = CCipasComposerArrow::MarkerMode( itemElem.attribute( "markerMode", "0" ).toInt() );
