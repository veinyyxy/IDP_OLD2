...
CCipasComposerScaleBar::CCipasComposerScaleBar(CCipasComposition *composition)
	: CComposerItem( composition )
	, mGLW( 0 ), mStyle( 0 ), mSegmentMillimeters( 0.0 )
{
...
CCipasComposerScaleBar::~CCipasComposerScaleBar(void)
{
	delete mStyle;
}


void CCipasComposerScaleBar::paint( QPainter* painter, const QStyleOptionGraphicsItem* itemStyle, QWidget* pWidget )
{
	if ( !mStyle || !painter )
	{
		return;
	}
	int iScale = ((CCipasComposerGLW*)mGLW)->GetMapScale();
...
void CCipasComposerScaleBar::setNumUnitsPerSegment( double units )
{
	mNumUnitsPerSegment = units;
	refreshSegmentMillimeters();
}

//void CCipasComposerScaleBar::setComposerMap( const QgsComposerMap* map )
//{
...
void CCipasComposerScaleBar::setComposerGLW(const CCipasComposerGLW* glw)
{
	//需要对 glw 进行操作
	disconnect( mGLW, SIGNAL( extentChanged() ), this, SLOT( updateSegmentSize() ) );
	disconnect( mGLW, SIGNAL( destroyed( QObject* ) ), this, SLOT( invalidateCurrentMap() ) );
	mGLW = glw;
	...
	connect( mGLW, SIGNAL( extentChanged() ), this, SLOT( updateSegmentSize() ) );
	connect( mGLW, SIGNAL( destroyed( QObject* ) ), this, SLOT( invalidateCurrentMap() ) );

	refreshSegmentMillimeters();
}

void CCipasComposerScaleBar::invalidateCurrentMap()
{
	disconnect( mGLW, SIGNAL( extentChanged() ), this, SLOT( updateSegmentSize() ) );
	disconnect( mGLW, SIGNAL( destroyed( QObject* ) ), this, SLOT( invalidateCurrentMap() ) );
	mGLW = 0;
...
void CCipasComposerScaleBar::refreshSegmentMillimeters()
{
	if ( mGLW )
	{
		//get extent of composer map
		//CCipasRectangle composerMapRect = mGLW->extent();
		CCipasRectangle composerMapRect =  mGLW->transformedExtent() ;

		//get mm dimension of composer map
		QRectF composerItemRect = mGLW->rect();
...
void CCipasComposerScaleBar::applyDefaultSettings()
{
	mNumSegments = 2;
	mNumSegmentsLeft = 0;
	...
	//style
	delete mStyle;
	mStyle = new CCipasSingleBoxScaleBarStyle( this );
...
void CCipasComposerScaleBar::applyDefaultSize()
{
	//if ( mComposerMap )
	//{
...
void CCipasComposerScaleBar::adjustBoxSize()
{
	if ( !mStyle )
	{
		return;
...
void CCipasComposerScaleBar::update()
{
	adjustBoxSize();
	CComposerItem::update();
}

void CCipasComposerScaleBar::updateSegmentSize()
{
	refreshSegmentMillimeters();
	update();
}

void CCipasComposerScaleBar::segmentPositions( QList<QPair<double, double> >& posWidthList ) const
{
	posWidthList.clear();
...
void CCipasComposerScaleBar::setStyle( const QString& styleName )
{
	delete mStyle;
	mStyle = 0;
	...
	{
		mStyle = new CipasDoubleBoxScaleBarStyle( this );
	}
	else if ( styleName == "Line Ticks Middle"  || styleName == "Line Ticks Down" || styleName == "Line Ticks Up" )
	{
		CipasTicksScaleBarStyle* tickStyle = new CipasTicksScaleBarStyle( this );
		if ( styleName == "Line Ticks Middle" )
		{
			tickStyle->setTickPosition( CipasTicksScaleBarStyle::TicksMiddle );
		}
		else if ( styleName == "Line Ticks Down" )
		{
			tickStyle->setTickPosition( CipasTicksScaleBarStyle::TicksDown );
		}
		else if ( styleName == "Line Ticks Up" )
		{
			tickStyle->setTickPosition( CipasTicksScaleBarStyle::TicksUp );
		}
		mStyle = tickStyle;
	}
	else if ( styleName == "Numeric" )
	{
		mStyle = new CipasNumericScaleBarStyle( this );
	}
}

QString CCipasComposerScaleBar::style() const
{
	if ( mStyle )
	{
		return mStyle->name();
...
QString CCipasComposerScaleBar::firstLabelString() const
{
...
QFont CCipasComposerScaleBar::font() const
{
	return mFont;
}

void CCipasComposerScaleBar::setFont( const QFont& font )
{
	mFont = font;
	adjustBoxSize();
	update();
...
bool CCipasComposerScaleBar::writeXML( QDomElement& elem, QDomDocument & doc ) const
{
	if ( elem.isNull() )
	{
		return false;
	...
	if ( mGLW )
	{
		composerScaleBarElem.setAttribute( "mapId", mGLW->id() );
	}
...
bool CCipasComposerScaleBar::readXML( const QDomElement& itemElem, const QDomDocument& doc )
{
	if ( itemElem.isNull() )
	{
		return false;
	...
	int GLWid = itemElem.attribute( "mapId", "-1" ).toInt();
	if ( GLWid >= 0 )
	{
		const CCipasComposerGLW* composerGLW = mComposition->getComposerGLWById( GLWid );
		mGLW = composerGLW;
		if ( composerGLW )
		{
			connect( composerGLW, SIGNAL( extentChanged() ), this, SLOT( updateSegmentSize() ) );
			connect( composerGLW, SIGNAL( destroyed( QObject* ) ), this, SLOT( invalidateCurrentMap() ) );
		}
	}
	refreshSegmentMillimeters();
