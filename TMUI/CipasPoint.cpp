...
CCipasVector::CCipasVector() : m_x( 0.0 ), m_y( 0.0 )
{
}

CCipasVector::CCipasVector( double x, double y ) : m_x( x ), m_y( y )
{
}

CCipasVector CCipasVector::operator-( void ) const
{
	return CCipasVector( -m_x, -m_y );
}

CCipasVector CCipasVector::operator*( double scalar ) const
{
	return CCipasVector( m_x * scalar, m_y * scalar );
}
...
double CCipasVector::operator*( CCipasVector v ) const
{
	return m_x * v.m_x + m_y * v.m_y;
}

double CCipasVector::length() const
{
	return sqrt( m_x * m_x + m_y * m_y );
}

double CCipasVector::x() const
{
	return m_x;
}

double CCipasVector::y() const
{
	return m_y;
}

// perpendicular vector (rotated 90?counter-clockwise)
CCipasVector CCipasVector::perpVector() const
{
	return CCipasVector( -m_y, m_x );
}

double CCipasVector::angle( void ) const
{
	double ang = atan2( m_y, m_x );
	return ang < 0.0 ? ang + 2.0 * M_PI : ang;
}

double CCipasVector::angle( CCipasVector v ) const
{
	return v.angle() - angle();
}

CCipasVector CCipasVector::rotateBy( double rot ) const
{
	double ang = atan2( m_y, m_x ) + rot;
	double len = length();
	return CCipasVector( len * cos( ang ), len * sin( ang ) );
}

CCipasVector CCipasVector::normal() const
{
	double len = length();

	if ( len == 0.0 )
	{
		//throw CipasException( "normal vector of null vector undefined" );
	}
...
CCipasPoint::CCipasPoint(void)
{
}


CCipasPoint::CCipasPoint( const CCipasPoint& p )
{
	m_x = p.x();
	m_y = p.y();
}
...
QString CCipasPoint::toString() const
{
	QString rep;
	QTextStream ot( &rep );
...
QString CCipasPoint::toString( int thePrecision ) const
{
...
QString CCipasPoint::toDegreesMinutesSeconds( int thePrecision ) const
{
...
QString CCipasPoint::wellKnownText() const
{
	return QString( "POINT(%1 %2)" ).arg( QString::number( m_x, 'f', 18 ) ).arg( QString::number( m_y, 'f', 18 ) );
}

double CCipasPoint::sqrDist( double x, double y ) const
{
	return ( m_x - x ) * ( m_x - x ) + ( m_y - y ) * ( m_y - y );
}

double CCipasPoint::sqrDist( const CCipasPoint& other ) const
{
	return sqrDist( other.x(), other.y() );
}

double CCipasPoint::azimuth( const CCipasPoint& other )
{
...
// operators
bool CCipasPoint::operator==( const CCipasPoint & other )
{
...
bool CCipasPoint::operator!=( const CCipasPoint & other ) const
{
	if (( m_x == other.x() ) && ( m_y == other.y() ) )
		return false;
	else
...
CCipasPoint & CCipasPoint::operator=( const CCipasPoint & other )
{
	if ( &other != this )
	{
		m_x = other.x();
...
void CCipasPoint::multiply( const double& scalar )
{
	m_x *= scalar;
	m_y *= scalar;
}

int CCipasPoint::onSegment( const CCipasPoint& a, const CCipasPoint& b ) const
{
...
double CCipasPoint::sqrDistToSegment( double x1, double y1, double x2, double y2, CCipasPoint& minDistPoint ) const
{
	...
	if (false /*doubleNear( dist, 0.0, 0.00000001 )*/ )
	{
